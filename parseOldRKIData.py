# parse a folder full of old rki publications
# This is a slightly modified version of `parseRKIData.py`
# Get the old rki publication from here: https://github.com/micb25/RKI_COVID19_DATA
...
def RKIparse(input_file, output_file_cases=output_file_cases, output_file_deaths=output_file_deaths):
	# create result matrices
	cases = np.zeros((len(AGS)+1,maximum_number_of_days_to_assume+1), dtype=int)
	day0 = date(2020,1,1).toordinal()  # ==737425, this is the first day
	...
	# open file
	import gzip
	with gzip.open(input_file, mode='rt', errors='ignore') as f:
	...
	# read column indexes
	header = [x.replace('"', '').strip() for x in lines[0]]
	new_cases_i = header.index('AnzahlFall')
	new_deaths_i = header.index('AnzahlTodesfall')
	ags_i = header.index('IdLandkreis')
	date_i = header.index('Meldedatum')
	...
	for i, entries in enumerate(lines):
		...
		try:
			...
			print(f'Error: {e}; skipping line {i}')
			error_count += 1
			if error_count >= 30:
...
def main(path):
	# go through all files in path and parse those looking like RKI data
	for filename in sorted(os.listdir(path)):
		if filename.startswith("RKI_COVID19_"):
			try:
				RKIparse(path+filename, f'cases/{filename[12:-3]}', f'deaths/{filename[12:-3]}')
			except Exception as e:
